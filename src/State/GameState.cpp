...
void GameState::enter() {
	...
	m_pSceneMgr
			= OgreFramework::getSingletonPtr()->m_pRoot->createSceneManager(
...
void GameState::exit() {
	...
	if (m_pSceneMgr)
		OgreFramework::getSingletonPtr()->m_pRoot->destroySceneManager(
				m_pSceneMgr);
}
...
bool GameState::keyPressed(const OIS::KeyEvent &keyEventRef) {
	if (m_bSettingsMode == true) {
		if (OgreFramework::getSingletonPtr()->m_pKeyboard->isKeyDown(OIS::KC_S)) {
			OgreBites::SelectMenu
					* pMenu =
							(OgreBites::SelectMenu*)
							OgreFramework::getSingletonPtr()->m_pTrayMgr->getWidget(
									"ChatModeSelMenu");
	...
	if (OgreFramework::getSingletonPtr()->m_pKeyboard->isKeyDown(OIS::KC_I)) {
		if (m_pDetailsPanel->getTrayLocation() == OgreBites::TL_NONE) {
			...
			OgreFramework::getSingletonPtr()->m_pTrayMgr->removeWidgetFromTray(
					m_pDetailsPanel);
	...
	if (m_bSettingsMode
			&& OgreFramework::getSingletonPtr()->m_pKeyboard->isKeyDown(
					OIS::KC_RETURN)
			|| OgreFramework::getSingletonPtr()->m_pKeyboard->isKeyDown(
					OIS::KC_NUMPADENTER)) {
	}
...
void GameState::onLeftPressed(const OIS::MouseEvent &evt) {
	...
	Ogre::Ray mouseRay = m_pCamera->getCameraToViewportRay(
			...
			OgreFramework::getSingletonPtr()->m_pMouse->getMouseState().Y.abs
	...
	for (itr = result.begin(); itr != result.end(); itr++) {
		if (itr->movable) {
			...
			OgreFramework::getSingletonPtr()->m_pLog->logMessage("ObjName "
...
void GameState::buildGUI() {
	...
	OgreFramework::getSingletonPtr()->m_pTrayMgr->showLogo(
